...
function lambda = int2lambdaETAS (Mi, Ti, Iri, starttime, Ml, theta, mub)
...
if ~isempty(Iri)
    ...
    lambda = mub + sum((K*exp(alpha*(Mi(index)-Ml))./((starttime-Ti(index)+c).^p)).*(Krt.*Iri(index)));
    
else
    
    Kt = (p-1)*c^(p-1);
    lambda = mub + sum(K*Kt*exp(alpha*(Mi(index)-Ml))./((starttime-Ti(index)+c).^p));
